...
19,20,20,21,22,23,23,20,22,24 // P
    ...
    var position = { x: -0.5, y: 0 }; // Initial position on the left edge
    var radian = 0; // Initial rotation angle
    var padding = 0.1;
        ...
        switch (direction) {
            case 'up':
                ...
                if (position.y > 1 - padding) {
                    ...
                    direction = 'right';
                    // radian = -Math.PI / 2; // Rotate 90 degrees to the right
                }
                break;
            case 'right':
                position.x += speed;
                if (position.x > 1 - padding) {
                    ...
                    direction = 'down';
                    // radian = Math.PI; // Rotate 180 degrees
                }
                break;
            case 'down':
                position.y -= speed;
                if (position.y < -0.9 + padding) {
                    position.y = -0.9 + padding;
                    direction = 'left';
                    // radian = -Math.PI / 2; // Rotate 270 degrees (or -90 degrees)
                }
                break;
            case 'left':
                position.x -= speed;
                if (position.x < -0.65 + padding) {
                    position.x = -0.65 + padding;
                    direction = 'up';
                    // radian = 0; // Reset rotation to 0 degrees
                }
                break;
        }
