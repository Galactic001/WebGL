var WebGL = function () {
	...
	[//  X, 	 Y,	 	Z		U, V
		// -0.7, 	-0.7,	0,	0.18, 0.0,  // V0 
		-0.9,	0.0,	0,	-0.01, 0.5,		// v1
		-0.7,	0.7,	0,	0.18, 1,		// v2
		0.7, 	0.7, 	0, 	0.825, 1,		// v3
		0.7, 	-0.7, 	0, 	0.825, 0,		// v4
		0.9, 	0.0, 	0, 	1.01, 0.5,		// v5

        // -0.7, 0.7, 0, 0.0, 1.0,  // Original corner
        ...verticesForCorner(-0.7, 0.7, cornerRadius, numSegments, 0.0, 1.0, 0.2, 0.8) // Modified vertices
	];  
...
function getShaderProgram(gl)
{
    ...
    void main()
    {
        vec3 pos = geometryCoordinatesVS;

        // Modify corner vertices if needed
        if (pos.x < -0.5 && pos.y > 0.5) { // Top-left corner
            float distFromCorner = length(pos + vec3(0.5, -0.5, 0.0));
            if (distFromCorner > cornerRadius) { 
                // Outside corner, no change
            } else { 
                float angle = acos(dot(normalize(pos + vec3(0.5, -0.5, 0.0)), vec3(1.0, 0.0, 0.0)));
                if (pos.y < 0.0) { angle = 2.0 * 3.14159 - angle; }
                pos.x = -0.5 + cornerRadius * cos(angle);
                pos.y = 0.5 - cornerRadius * sin(angle);
            }
        } 
        // ... Similarly modify other corners 
    ...
    void main()
    {
        float distFromCenter = length(textureInfoVS_for_FS - vec2(0.5, 0.5));
        if (distFromCenter > 0.5 - cornerRadius) {
            discard; 
        } else {
...
function verticesForCorner(x, y, cornerRadius, numSegments, uStart, vStart, uEnd, vEnd) {
    var vertices = [];
    for (let i = 0; i <= numSegments; i++) {
        let theta = i / numSegments * Math.PI / 2;
        let vx = x + cornerRadius * Math.cos(theta);
        let vy = y - cornerRadius * Math.sin(theta);
        let u = uStart + (uEnd - uStart) * (1 - Math.cos(theta)) / 2.0;
        let v = vStart + (vEnd - vStart) * (1 - Math.sin(theta)) / 2.0;
        vertices.push(vx, vy, 0, u, v);
    }
    return vertices;
}